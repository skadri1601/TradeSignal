...
class ScrapeHistoryCreate(ScrapeHistoryBase):
    ...
    status: str = Field(
        "running", max_length=20, description="Status: running, success, failed"
    )
...
class ScrapeHistoryUpdate(BaseModel):
    ...
    status: Optional[str] = Field(
        None, max_length=20, description="Status: success or failed"
    )
    filings_found: Optional[int] = Field(
        None, ge=0, description="Number of filings found"
    )
    trades_created: Optional[int] = Field(
        None, ge=0, description="Number of trades created"
    )
