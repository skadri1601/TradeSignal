...
async def get_companies(
    ...
    **Query Parameters:**
    ...
    companies, total = await CompanyService.get_all(
        db=db, skip=pagination.skip, limit=pagination.limit, sector=sector
    )
...
async def get_company_logo(
    request: Request, ticker: str, db: AsyncSession = Depends(get_db)
):
    """
    ...
    **Parameters:**
    ...
    Returns logo URL from Yahoo Finance. Result is cached for 24 hours.
    """
    profile_service = CompanyProfileService(db)
    logo_url = await profile_service.get_company_logo(ticker.upper())

    if not logo_url:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail=f"Logo not found for ticker '{ticker}'",
        )
...
async def get_company_by_ticker(
    ...
    """
    Get company by ticker symbol with statistics and live data.
    ...
    **Parameters:**
    ...
    - Live market cap (updated from current stock price)
    - Logo URL (if available)

    **Auto-enrichment**: Company data is automatically enriched from Yahoo Finance and SEC EDGAR
    if missing or stale (>7 days old). This includes logo, sector, industry, and market cap.
    """
    ...
    # Auto-enrich company data if missing or stale
    enrichment_service = CompanyEnrichmentService(db)
    await enrichment_service.enrich_company(company.id)

    # Refresh company from DB to get updated data
    await db.refresh(company)

    # Get live market cap from current stock price
    quote = StockPriceService.get_stock_quote(ticker.upper(), use_cache=True)
    if quote and quote.get("market_cap"):
        # Update market cap if we have live data
        if not company.market_cap or quote["market_cap"] != company.market_cap:
...
async def create_company(
    request: Request, company_data: CompanyCreate, db: AsyncSession = Depends(get_db)
):
    """
    ...
    **Note:** Ticker and CIK must be unique.
    ...
    # Auto-enrich newly created company with Yahoo Finance data
    enrichment_service = CompanyEnrichmentService(db)
    await enrichment_service.enrich_company(company.id)
...
async def update_company(
    ...
    updated_company = await CompanyService.update(
        db=db, company=company, company_data=company_data
    )
