...
@celery_app.task(name="precalculate_risk_levels_batch")
def precalculate_risk_levels_batch_task(tickers: List[str]):
    """
    Pre-calculate risk levels for a batch of tickers.

    Args:
        tickers: List of ticker symbols to process
    """
    async def _async_task():
        logger.info(f"Starting risk level batch calculation for {len(tickers)} tickers")
        ...
        async with db_manager.get_session() as db:
            ...
            for ticker in tickers:
                try:
                    ...
                    # Calculate risk level (will use default/placeholder values if financial data not available)
                    calculation_result = await risk_service.calculate_risk_level(ticker=ticker)

                    # Save to database
                    risk_assessment = RiskLevelAssessment(
                        ticker=ticker,
                        risk_level=calculation_result["risk_level"],
                        composite_score=calculation_result["score"],
                        earnings_volatility_score=calculation_result["component_scores"]["earnings_volatility"],
                        financial_leverage_score=calculation_result["component_scores"]["financial_leverage"],
                        operating_leverage_score=calculation_result["component_scores"]["operating_leverage"],
                        concentration_score=calculation_result["component_scores"]["concentration"],
                        industry_stability_score=calculation_result["component_scores"]["industry_stability"],
                        calculated_at=datetime.utcnow(),
                    )
                    ...
                    # Invalidate cache
                    await cache_service.delete(f"risk_level:{ticker}:latest")

                    results["succeeded"] += 1
                    logger.debug(f"Successfully calculated risk level for {ticker}")
                    ...
                    error_msg = f"{ticker}: {str(e)}"
                    results["errors"].append(error_msg)
                    logger.error(f"Error calculating risk level for {ticker}: {e}", exc_info=True)

            logger.info(
                f"Risk level batch calculation complete: "
                f"{results['succeeded']} succeeded, {results['failed']} failed out of {results['processed']} processed"
            )
            return results
...
@celery_app.task(name="batch_calculate_risk_levels")
def batch_calculate_risk_levels_task():
    """
    Calculate risk levels for popular tickers daily at 4 AM UTC.

    Process:
    1. Get list of popular tickers (top 100 by insider activity)
    2. For each ticker, calculate risk level using RiskLevelService
    3. Cache results for 24 hours
    4. Log progress and errors
    """
    async def _async_task():
        logger.info("Starting daily risk level calculation for popular tickers")

        async with db_manager.get_session() as db:
            # Get popular tickers based on insider trading activity (last 90 days)
            cutoff_date = datetime.utcnow() - timedelta(days=90)
            result = await db.execute(
                select(Company.ticker, func.count(Trade.id).label("trade_count"))
                .join(Trade, Company.id == Trade.company_id)
            ...
            logger.info(f"Found {len(popular_tickers)} popular tickers for risk level calculation")

            # Process in batches
            batch_size = settings.batch_processing_batch_size or 10
            total_succeeded = 0
            total_failed = 0
            total_processed = 0
            ...
            for i in range(0, len(popular_tickers), batch_size):
                ...
                batch_results = precalculate_risk_levels_batch_task.delay(batch).get(timeout=300)
                total_succeeded += batch_results.get("succeeded", 0)
                total_failed += batch_results.get("failed", 0)
                total_processed += batch_results.get("processed", 0)

            summary = {
                "processed": total_processed,
                "succeeded": total_succeeded,
                "failed": total_failed,
                "duration_seconds": 0  # Will be calculated by Celery
            }

            logger.info(
                f"Daily risk level calculation complete: "
                f"{total_succeeded} succeeded, {total_failed} failed out of {total_processed} processed"
            )
            return summary
